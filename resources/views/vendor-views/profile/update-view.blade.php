...
<div class="content container-fluid">
    ...
    <div class="row">
            ...
            <div class="navbar-vertical navbar-expand-lg mb-3 mb-lg-5">
                ...
                <div id="navbarVerticalNavMenu" class="collapse navbar-collapse">
                    <ul id="navbarSettings" class="js-sticky-block js-scrollspy navbar-nav navbar-nav-lg nav-tabs card card-navbar-nav">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="javascript:" id="plan-section">
                                <img width="15" src="{{dynamicAsset(path: 'public/assets/back-end/img/box.png')}}" class="mr-2" alt=""> {{translate('plan')}}
                            </a>
                        </li>
                        @endif
                    </ul>
        ...
        <div class="col-lg-9">
            <form action="{{route('vendor.profile.update',[$vendor->id])}}" method="post" enctype="multipart/form-data" id="update-profile-form">
                @csrf
                <div class="card mb-3 mb-lg-5" id="general-div">
                    ...
                    <div class="avatar avatar-xxl avatar-circle avatar-border-lg avatar-uploader profile-cover-avatar">
                        <img id="viewer" class="avatar-img" src="{{getValidImage(path:'storage/app/public/seller/'.$vendor->image, type:'backend-profile')}}" alt="{{translate('image')}}">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <label for="firstNameLabel" class="col-sm-3 col-form-label input-label">{{translate('full_Name')}}
                                <i class="tio-help-outlined text-body ml-1" data-toggle="tooltip" data-placement="right" title="{{ucwords($vendor->f_name.' '.$vendor->l_name)}}"></i>
                            </label>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-9 mb-3">
                                <input class="form-control form-control-user phone-input-with-country-picker" type="tel" id="exampleInputPhone" value="{{$vendor->phone ?? old('phone')}}" placeholder="{{ translate('enter_phone_number') }}" required>
                                <div class="">
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-sm-9">
                                <input type="email" class="form-control" name="email" id="newEmailLabel" value="{{$vendor->email}}" placeholder="{{translate('enter_new_email_address')}}" readonly>
                            </div>
                        </div>
                        <div class="row form-group">
                            <label for="newGenderLabel" class="col-sm-3 col-form-label input-label">{{translate('gender')}}</label>

                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="newGenderLabel" value="{{$vendor->gender}}" placeholder="{{translate('enter_gender')}}" readonly>
                            </div>
                        </div>
                        <div class="row form-group">
                            <label for="newYears21Label" class="col-sm-3 col-form-label input-label">{{translate('years21')}}</label>

                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="newYears21Label" value="{{$vendor->years21}}" placeholder="{{translate('enter_years21')}}" readonly>
                            </div>
                        </div>
            ...
            <div id="password-div" class="card mb-3 mb-lg-5">
                ...
                <div class="card-body">
                    <form id="update-password-form" action="{{route('vendor.profile.update',[auth('seller')->id()])}}" method="POST" enctype="multipart/form-data">
                        @method('PATCH')
                        @csrf
                        <div class="row form-group">
                            ...
                            <div class="col-sm-9">
                                <div class="input-group input-group-merge">
                                    <input type="password" class="js-toggle-password form-control password-check" id="newPassword" autocomplete="off" name="password" required minlength="8" placeholder="{{ translate('password_minimum_8_characters') }}" data-hs-toggle-password-options='{"target": "#changePassTarget", "defaultClass": "tio-hidden-outlined", "showClass": "tio-visible-outlined", "classChangeTarget": "#changePassIcon"}'>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-sm-9">
                                <div class="mb-3">
                                    <div class="input-group input-group-merge">
                                        <input type="password" class="js-toggle-password form-control" name="confirm_password" required id="confirmNewPasswordLabel" placeholder="{{ translate('confirm_password') }}" autocomplete="off" data-hs-toggle-password-options='{"target": "#changeConfirmPassTarget", "defaultClass": "tio-hidden-outlined", "showClass": "tio-visible-outlined", "classChangeTarget": "#changeConfirmPassIcon"}'>
            ...
            @if(auth()->guard('seller')->check())
            <div id="plan-div" class="card mb-3 mb-lg-5">
                <div class="card-header">
                    <h5 class="mb-0">{{ translate('change_your_plan') }}</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <div class="card-deck mb-3 text-center">
                                ...
                                <div class="card card-plan mb-4 shadow-sm" id="plan-{{ $plan->id }}">
                                    <div class="card-header bg-primary d-flex justify-content-center">
                                        <h5 class="text-white font-weight-bold my-0">{{ $plan->name }}</h5>
                                    </div>
                                    <div class="card-body">
                                        <h3 class="card-title pricing-card-title">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $plan->price)) }}<small class="text-muted">/@if($plan->duration==1){{ translate('month') }}@endif</small></h3>
                                        <ul class="list-unstyled mt-3 mb-4">
                                            <li>{{ $plan->sales_commission }}% {{ translate('commission_to_supplier') }}</li>
                                        </ul>

                                        @if($vendor->subscribedToPrice($plan->subscription_api_id))
                                        <p class="text-center font-weight-bold">
                                            <span class="badge badge-primary">{{ translate('active_plan') }}</span>
                                        </p>
                                        @else
                                        <a href="{{ route('vendor.profile.checkout-subscription', ['plan' => $plan->id]) }}" class="btn btn-block btn--primary">{{ translate('select') }}</a>
                                        @endif
                                    </div>
                                </div>
                                @endforeach
