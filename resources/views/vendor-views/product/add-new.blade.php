...
<div class="content container-fluid">
    <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
        <h2 class="h1 mb-0 d-flex gap-2">
            <img src="{{ dynamicAsset(path: 'public/assets/back-end/img/inhouse-product-list.png') }}" alt=""> {{ translate('add_New_Product') }}
        </h2>
    </div>

    <form class="product-form text-start" action="{{ route('vendor.products.add') }}" method="POST" enctype="multipart/form-data" id="product_form">
        @csrf
        <div class="card">
            <div class="px-4 pt-3">
                ...
                <div class="row">
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            ...
                            <select class="js-select2-custom form-control action-get-request-onchange" name="category_id" data-url-prefix="{{ route('vendor.products.get-categories').'?parent_id=' }}" data-element-id="sub-category-select" data-element-type="select" required>
                                <option value="{{ old('category_id') }}" selected disabled>{{ translate('select_category') }}</option>
                                @foreach ($categories as $category)
                                <option value="{{ $category['id'] }}" {{ old('name') == $category['id'] ? 'selected' : '' }}>{{ $category['defaultName'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            ...
                            <select class="js-select2-custom form-control action-get-request-onchange" name="sub_category_id" id="sub-category-select" data-url-prefix="{{ route('vendor.products.get-categories').'?parent_id=' }}" data-element-id="sub-sub-category-select" data-element-type="select">
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            ...
                            <select class="js-select2-custom form-control" name="sub_sub_category_id" id="sub-sub-category-select">
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            ...
                            <input type="text" minlength="6" id="generate_number" name="code" class="form-control" value="{{ old('code') }}" placeholder="{{ translate('123412') }}" required>
                        </div>
                    </div>
                ...
                <div class="row align-items-end">
                    <div class="form-group col-12">
                        ...
                        <select name="type_price" class="form-control" required id="type_price">
                            <option value="1" @if(is_null(old('type_price')) || old('type_price')=='1') selected @endif>{{ translate('unit_price') }}</option>
                            <option value="2" @if(old('type_price')=='2') selected @endif>{{ translate('quantity_price') }}</option>
                        </select>
                    </div>

                    <div class="col-12 @if(!is_null(old('type_price')) && old('type_price')!='1') d-none @endif" id="container_unit_price">
                        <div class="row">
                    ...
                    <div class="col-12 @if(is_null(old('type_price')) || (!is_null(old('type_price')) && old('type_price')!='2')) d-none @endif" id="container_quantity_price">
                        ...
                        <div class="row">
                            <div class="col-12" id="price_elements">
                                @if(!is_null(old('prices')))
                                @foreach(old('prices') as $key => $price)
                                <div class="row">
                                    <div class="form-group col-lg-4 col-md-4 col-12">
                                        <label class="title-color mb-2">{{ translate('minimum_quantity') }}</label>
                                        <input type="text" name="prices[{{ $key }}][qty_min]" class="form-control @if(!$loop->first) integer-positive @else bg-light @endif" placeholder="{{ translate('minimum_quantity') }}" required @if(!$loop->first) readonly @endif min="1" value="{{ old('prices.'.$key.'.qty_min', 1) }}" data-index="{{ $key }}" id="{{ 'prices_qty_min_'.$key }}">
                                    </div>

                                    <div class="form-group col-lg-4 col-md-4 col-12">
                                        <label class="title-color mb-2">{{ translate('maximum_quantity') }}</label>
                                        <input type="text" name="prices[{{ $key }}][qty_max]" class="form-control integer-positive" placeholder="{{ translate('maximum_quantity') }}" required min="1" value="{{ old('prices.'.$key.'.qty_max', 1) }}" data-index="{{ $key }}" id="{{ 'prices_qty_max_'.$key }}">
                                    </div>

                                    <div class="form-group col-lg-4 col-md-4 col-12">
                                        <label class="title-color mb-2">{{ translate('unit_price') }} ({{ getCurrencySymbol(currencyCode: getCurrencyCode()) }})</label>
                                        <input type="text" name="prices[{{ $key }}][price]" class="form-control min-decimal" placeholder="{{ translate('unit_price') }}" required min="0" value="{{ old('prices.'.$key.'.price', 0.00) }}" data-index="{{ $key }}" id="{{ 'prices_price_'.$key }}">
                                    </div>
                                ...
                                <div class="row">
                                    <div class="form-group col-lg-4 col-md-4 col-12">
                                        <label class="title-color mb-2">{{ translate('minimum_quantity') }}</label>
                                        <input type="text" name="prices[0][qty_min]" class="form-control integer-positive" placeholder="{{ translate('minimum_quantity') }}" required min="1" value="{{ old('prices.0.qty_min', 1) }}" data-index="0" id="prices_qty_min_0">
                                    </div>

                                    <div class="form-group col-lg-4 col-md-4 col-12">
                                        <label class="title-color mb-2">{{ translate('maximum_quantity') }}</label>
                                        <input type="text" name="prices[0][qty_max]" class="form-control integer-positive" placeholder="{{ translate('maximum_quantity') }}" required min="1" value="{{ old('prices.0.qty_max', 1) }}" data-index="0" id="prices_qty_max_0">
                                    </div>

                                    <div class="form-group col-lg-4 col-md-4 col-12">
                                        <label class="title-color mb-2">{{ translate('unit_price') }} ({{ getCurrencySymbol(currencyCode: getCurrencyCode()) }})</label>
                                        <input type="text" name="prices[0][price]" class="form-control min-decimal" placeholder="{{ translate('unit_price') }}" required min="0" value="{{ old('prices.0.price', 0.00) }}" data-index="0" id="prices_price_0">
                                    </div>
                    ...
                    <div class="form-group col-md-6 col-lg-4 col-xl-3 d-none">
                        <div class="">
                            <div class="d-flex gap-2 mb-2">
                                <label class="title-color mb-0">{{ translate('purchase_price') }} ({{ getCurrencySymbol(currencyCode: getCurrencyCode()) }})</label>
                            ...
                            <input type="number" min="0" step="0.01" placeholder="{{ translate('purchase_price') }}" value="{{ old('purchase_price') }}" name="purchase_price" class="form-control" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            <div class="d-flex gap-2">
                                <label class="title-color" for="discount">{{ translate('discount_amount') }} <span class="discount_amount_symbol">({{ getCurrencySymbol(currencyCode: getCurrencyCode()) }})</span></label>
                            ...
                            <input type="number" min="0" value="0" step="0.01" placeholder="{{ translate('ex: 5') }}" name="discount" id="discount" class="form-control" required>
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            ...
                            <input type="number" min="0" step="0.01" placeholder="{{ translate('ex: 5') }}" name="tax" id="tax" value="{{ old('tax') ?? 0 }}" class="form-control">
                ...
                <div class="row align-items-end">
                    <div class="col-md-6">
                        <div class="mb-3 d-flex align-items-center gap-2">
                            <label for="colors" class="title-color mb-0">{{ translate('select_colors') }} :</label>
                            <label class="switcher">
                                <input type="checkbox" class="switcher_input" id="product-color-switcher" value="{{ old('colors_active') }}" name="colors_active">
                                <span class="switcher_control"></span>
                            </label>
                        </div>
                        <select class="js-example-basic-multiple js-states js-example-responsive form-control color-var-select" name="colors[]" multiple="multiple" id="colors-selector" disabled>
                    ...
                    <div class="col-md-6">
                        <label for="choice_attributes" class="title-color">{{ translate('select_attributes') }} :</label>
                                ...
                                <div>
                                    <div class="custom_upload_input">
                                        ...
                                        <div class="img_area_with_preview position-absolute z-index-2">
                                            <img id="pre_img_viewer" class="h-auto aspect-1 bg-white d-none" src="" alt="">
                                        </div>
                            ...
                            <div class="row g-2" id="additional_Image_Section">
                                <div class="col-sm-12 col-md-4">
                                    <div class="custom_upload_input position-relative border-dashed-2 aspect-1">
                                        <input type="file" name="images[]" class="custom-upload-input-file action-add-more-image" data-index="1" data-imgpreview="additional_Image_1" accept=".jpg, .png, .webp, .jpeg, .gif, .bmp, .tif, .tiff|image/*"
