        ...
        <div class="card __card mb-3">
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-sm-6">
                        <select name="gender" class="form-control form-control-user" required id="exampleInputGender">
                    ...
                    <div class="col-sm-6">
                        <select name="years21" class="form-control form-control-user" required id="exampleInputYears21">
                            <option value="">{{ translate('years21') }}</option>
                            <option value="0" @if(old('years21')=='0') selected @endif>{{ translate('no') }}</option>
                            <option value="1" @if(old('years21')=='1') selected @endif>{{ translate('yes') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="text-center">
                            <img class="__img-125px object-cover" id="viewer" src="{{ getValidImage(path: 'public/assets/front-end/img/placeholder/user.png', type: 'avatar') }}" alt="banner image"/>
                        </div>
                        <div class="custom-file mt-3">
                            <input type="file" name="image" id="custom-file-upload" class="custom-file-input image-preview-before-upload" data-preview="#viewer" accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                            <label class="custom-file-label overflow-hidden text-truncate" for="custom-file-upload">{{ translate('upload_image') }}</label>
                        </div>
                    </div>

                    <div class="col-sm-6">
                        <div class="text-center">
                            <img class="__img-125px object-cover" id="viewerDocument" src="{{ getValidImage(path: 'public/assets/front-end/img/placeholder/placeholder-1-1.png', type: 'backend-basic') }}" alt="document image"/>
                        </div>
                        <div class="custom-file mt-3">
                            <input type="file" name="document" id="document-upload" class="custom-file-input image-preview-before-upload" data-preview="#viewerDocument" accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                            <label class="custom-file-label overflow-hidden text-truncate" for="document-upload">{{ translate('upload_document_identity') }}</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        <div class="pb-3">
                            <div class="text-center">
                                <img class="height-100px" id="viewerConstancy" src="{{ getValidImage(path: 'public/assets/front-end/img/placeholder/placeholder-1-1.png', type: 'backend-basic') }}" alt="constancy image"/>
                            </div>
                        </div>

                        <div class="form-group">
                            <div class="custom-file">
                                <input type="file" name="constancy" id="constancy-upload" class="custom-file-input overflow-hidden __p-2p image-preview-before-upload" data-preview="#viewerConstancy" accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                <label class="custom-file-label overflow-hidden text-truncate" for="constancy-upload">{{ translate('upload_constancy') }}</label>
                            </div>
                        </div>
                    </div>

                    <div class="col-12">
                        <h5 class="text-center font-weight-bold mb-4">{{ translate('select_a_plan') }}</h5>

                        <div class="card-deck mb-3 text-center">
                            @foreach($plans as $plan)
                            <div class="card card-plan @if($loop->first) bg-secondary @endif mb-4 shadow-sm" id="plan-{{ $plan->id }}">
                                <div class="card-header">
                                    <h5 class="my-0 font-weight-normal">{{ $plan->name }}</h5>
                                </div>
                                <div class="card-body">
                                    <h4 class="card-title pricing-card-title">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $plan->price)) }}<small class="text-muted">/@if($plan->duration==1){{ translate('month') }}@endif</small></h4>
                                    <ul class="list-unstyled mt-3 mb-4">
                                        <li>{{ $plan->sales_commission }}% {{ translate('commission_to_supplier') }}</li>
                                    </ul>
                                    <button type="button" class="btn btn-sm btn-block btn--primary select-plan" data-id="{{ $plan->id }}" data-select="{{ translate('select') }}" data-selected="{{ translate('selected') }}">@if($loop->first){{ translate('selected') }}@else{{ translate('select') }}@endif</button>
                                </div>
                            </div>
                            @endforeach
                        </div>

                        <input type="hidden" name="plan" value="{{ $plans->first()->id }}" id="input-plan">
                    </div>
                    ...
                    <div class="col-12">
                        <div class="row py-2">
                            ...
                            <div class="col-6 input-icons mb-2 w-100 rounded bg-white">
                                <span class="d-flex align-items-center align-items-center get-vendor-regi-recaptcha-verify" data-link="{{ route('vendor.auth.recaptcha', ['tmp'=>':dummy-id']) }}">
                        ...
                        <div class="form-group mb-0 d-flex flex-wrap justify-content-between">
                            <label class="form-group mb-3 d-flex align-items-center flex-grow-1 cursor-pointer user-select-none">
                                ...
                                <span class="mb-4px d-block w-0 flex-grow pl-1">
                                    ...
                                    </a> | <a class="font-size-sm text-primary text-force-underline" target="_blank" href="{{ route('privacy-policy') }}">{{ translate('privacy_policy') }}</a>
                                </span>
                            </label>
                        </div>
