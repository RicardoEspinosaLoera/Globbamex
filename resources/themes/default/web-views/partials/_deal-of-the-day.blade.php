                ...
                <div class="recommended-product-card mt-0 min-height-auto">
                    <div class="d-flex justify-content-center align-items-center __pt-20 __m-20-r">
                        <div class="position-relative">
                            <img class="__rounded-top aspect-1 h-auto" alt="" src="{{ getValidImage(path: 'storage/app/public/product/thumbnail/'.$deal_of_the_day->product['thumbnail'], type: 'product') }}">
                    ...
                    <div class="__i-1 bg-transparent text-center mb-0">
                        <div class="px-0">
                            ...
                            <div class="mb-4 pt-1 d-flex flex-wrap justify-content-center align-items-center text-center gap-8">
                                @if($deal_of_the_day->product->type_price=='2')
                                @if($deal_of_the_day->product->discount>0)
                                <del class="fs-14 font-semibold __color-9B9B9B">
                                    @if(isset($deal_of_the_day->product['prices']) && isset($deal_of_the_day->product['prices'][0]) && isset($deal_of_the_day->product['prices'][0]['price']))
                                    {{ webCurrencyConverter(amount: $deal_of_the_day->product['prices'][0]['price']) }}
                                    @else
                                ...
                                <span class="text-accent fs-18 font-bold text-dark">
                                    @if(isset($deal_of_the_day->product['prices']) && isset($deal_of_the_day->product['prices'][0]) && isset($deal_of_the_day->product['prices'][0]['price']))
                                    {{ webCurrencyConverter(amount: $deal_of_the_day->product['prices'][0]['price'] - getProductDiscount(product: $deal_of_the_day->product, price: $deal_of_the_day->product['prices'][0]['price'])) }}
                                    @else
                                    {{ webCurrencyConverter(amount: $deal_of_the_day->product->unit_price - getProductDiscount(product: $deal_of_the_day->product, price: $deal_of_the_day->product->unit_price)) }}
                                    @endif
                                </span>
                                @else
                                @if($deal_of_the_day->product->discount>0)
                                ...
                                @endif
                                <span class="text-accent fs-18 font-bold text-dark">{{ webCurrencyConverter(amount: $deal_of_the_day->product->unit_price - (getProductDiscount(product: $deal_of_the_day->product, price: $deal_of_the_day->product->unit_price))) }}</span>
                                @endif
                            </div>
                            <button class="btn btn--primary font-bold px-4 rounded-10 text-uppercase get-view-by-onclick" data-link="{{ route('product',$deal_of_the_day->product->slug) }}">{{translate('buy_now')}}</button>
                        </div>
                    </div>
                </div>
                @else
                ...
                <div class="recommended-product-card mt-0">
                    ...
                    <div class="__i-1 bg-transparent text-center mb-0 min-height-auto">
                        <div class="px-0 pb-0">
                            ...
                            <div class="mb-4 pt-1 d-flex flex-wrap justify-content-center align-items-center text-center gap-8">
                                @if($product->type_price=='2')
                                @if($product->discount>0)
                                <del class="__text-12px __color-9B9B9B">
                                    @if(isset($product['prices']) && isset($product['prices'][0]) && isset($product['prices'][0]['price']))
                                    {{ webCurrencyConverter(amount: $product['prices'][0]['price']) }}
                                    @else
                                ...
                                <span class="text-accent __text-22px text-dark">
                                    @if(isset($product['prices']) && isset($product['prices'][0]) && isset($product['prices'][0]['price']))
                                    {{ webCurrencyConverter(amount: $product['prices'][0]['price'] - getProductDiscount(product: $product, price: $product['prices'][0]['price'])) }}
                                    @else
                                ...
                                @endif
                                <span class="text-accent __text-22px text-dark">{{ webCurrencyConverter(amount: $product->unit_price - (getProductDiscount(product: $product, price: $product->unit_price))) }}</span>
                                @endif
                            </div>
                            <button class="btn btn--primary font-bold px-4 rounded-10 text-uppercase get-view-by-onclick" data-link="{{ route('product',$product->slug) }}">{{translate('buy_now')}}</button>
                        </div>
                    </div>
                </div>
                @endif
