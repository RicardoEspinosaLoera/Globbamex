    ...
    <div class="card card __shadow h-100">
        <div class="card-body p-xl-35">
            <div class="row d-flex justify-content-between mx-1 mb-3">
                <div>
                    <img class="size-30" src="{{theme_asset(path: "public/assets/front-end/png/top-rated.png")}}" alt="">
                    <span class="font-bold pl-1">{{ translate('top_rated')}}</span>
                </div>
                <div>
                    <a class="text-capitalize view-all-text web-text-primary" href="{{route('products',['data_from'=>'top-rated','page'=>1])}}">{{ translate('view_all')}}
                        ...
                        <div class="d-flex flex-wrap">
                            ...
                            <div class="top-rated-details">
                                ...
                                <div class="widget-product-meta d-flex flex-wrap gap-8 align-items-center row-gap-0">
                                    @if($top->product->type_price=='2')
                                    <span>
                                        @if($top->product->discount>0)
                                        <del class="__text-12px __color-9B9B9B">
                                            @if(isset($top->product['prices']) && isset($top->product['prices'][0]) && isset($top->product['prices'][0]['price']))
                                            {{ webCurrencyConverter(amount: $top->product['prices'][0]['price']) }}
                                            @else
                                    ...
                                    <span class="text-accent text-dark">
                                        @if(isset($top->product['prices']) && isset($top->product['prices'][0]) && isset($top->product['prices'][0]['price']))
                                        {{ webCurrencyConverter(amount: $top->product['prices'][0]['price'] - getProductDiscount(product: $top->product, price: $top->product['prices'][0]['price'])) }}
                                        @else
                                        {{ webCurrencyConverter(amount: $top->product->unit_price - getProductDiscount(product: $top->product, price: $top->product->unit_price)) }}
                                        @endif
                                    </span>
                                    @else
                                    <span>
                                    ...
                                    <span class="text-accent text-dark">{{ webCurrencyConverter(amount: $top->product->unit_price - (getProductDiscount(product: $top->product, price: $top->product->unit_price))) }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
