    ...
    <div class="card card __shadow h-100">
        <div class="card-body p-xl-35">
            <div class="row d-flex justify-content-between mx-1 mb-3">
                <div>
                    <img class="size-30" src="{{theme_asset(path: "public/assets/front-end/png/best-sellings.png")}}" alt="">
                    <span class="font-bold pl-1">{{ translate('best_sellings')}}</span>
                </div>
                <div>
                    <a class="text-capitalize view-all-text web-text-primary" href="{{route('products',['data_from'=>'best-selling','page'=>1])}}">{{ translate('view_all')}}
                        ...
                        <div class="d-flex flex-wrap">
                            ...
                            <div class="best-selling-details">
                                ...
                                <div class="widget-product-meta d-flex flex-wrap gap-8 align-items-center row-gap-0">
                                    @if($bestSell->product->type_price=='2')
                                    <span>
                                        @if($bestSell->product->discount>0)
                                        <del class="__color-9B9B9B __text-12px">
                                            @if(isset($bestSell->product['prices']) && isset($bestSell->product['prices'][0]) && isset($bestSell->product['prices'][0]['price']))
                                            {{ webCurrencyConverter(amount: $bestSell->product['prices'][0]['price']) }}
                                            @else
                                    ...
                                    <span class="text-accent text-dark">
                                        @if(isset($bestSell->product['prices']) && isset($bestSell->product['prices'][0]) && isset($bestSell->product['prices'][0]['price']))
                                        {{ webCurrencyConverter(amount: $bestSell->product['prices'][0]['price'] - getProductDiscount(product: $bestSell->product, price: $bestSell->product['prices'][0]['price'])) }}
                                        @else
                                        {{ webCurrencyConverter(amount: $bestSell->product->unit_price - getProductDiscount(product: $bestSell->product, price: $bestSell->product->unit_price)) }}
                                        @endif
                                    </span>
                                    @else
                                    <span>
                                    ...
                                    <span class="text-accent text-dark">{{ webCurrencyConverter(amount: $bestSell->product->unit_price-(getProductDiscount(product: $bestSell->product, price: $bestSell->product->unit_price))) }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
