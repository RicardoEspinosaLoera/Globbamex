...
$(document).on('ready', function () {
    ...
    // Touchspin
    if ($('.integer-positive').length) {
        $(".integer-positive").TouchSpin({
            min: 1,
            max: 999999999,
            step: 1,
            decimals: 0,
...
// Change type of price in products
$('#type_price').on('change', function() {
    if ($(this).val()=='1') {
        $('#container_quantity_price').addClass('d-none');
        $('#container_unit_price').removeClass('d-none');
    } else if ($(this).val()=='2') {
        $('#container_unit_price').addClass('d-none');
        $('#container_quantity_price').removeClass('d-none');
    } else {
        $('#container_quantity_price').addClass('d-none');
        $('#container_unit_price').removeClass('d-none');
    }
...
// Add price in products
function addPrice(text_min, text_max, text_price, price_symbol) {
    let count=$("#price_elements > div").length;

    $("#price_elements").append('<div class="row">'+
    '<div class="form-group col-lg-4 col-md-4 col-12">'+
    '<label class="title-color mb-2">'+text_min+'</label>'+
    '<input type="text" name="prices['+count+'][qty_min]" class="form-control bg-light" placeholder="'+text_min+'" required readonly min="1" value="1" data-index="'+count+'" id="prices_qty_min_'+count+'">'+
    '</div>'+

    '<div class="form-group col-lg-4 col-md-4 col-12">'+
    '<label class="title-color mb-2">'+text_max+'</label>'+
    '<input type="text" name="prices['+count+'][qty_max]" class="form-control integer-positive" placeholder="'+text_max+'" required min="1" value="1" data-index="'+count+'" id="prices_qty_max_'+count+'">'+
    '</div>'+

    '<div class="form-group col-lg-4 col-md-4 col-12">'+
    '<label class="title-color mb-2">'+text_price+' ('+price_symbol+')</label>'+
    '<input type="text" name="prices['+count+'][price]" class="form-control min-decimal" placeholder="'+text_price+'" required min="0" value="0.00" data-index="'+count+'" id="prices_price_'+count+'">'+
    '</div>'+
    ...
    changeQtyMin($('input[name="prices['+(count-1)+'][qty_max]"]'));

    $('input[name^="prices"][name$="[qty_max]"]').on('change', function() {
        changeQtyMin($(this));
    }).on('keyup', function() {
        changeQtyMin($(this));
    });

    count=$("#price_elements > div").length;
    if (count>1) {
        $('#remove_price').removeClass('d-none');
    }
};

// Remove price in products
function removePrice() {
    var count=$("#price_elements > div").length;
    if (count>1) {
        $("#price_elements > div:last-child").remove();
    }
    count=$("#price_elements > div").length;
    if (count==1) {
        $('#remove_price').addClass('d-none');
    }
}

// Function for change the min quantity
$('input[name^="prices"][name$="[qty_max]"]').on('change', function() {
    changeQtyMin($(this));
}).on('keyup', function() {
    changeQtyMin($(this));
...
function changeQtyMin(element) {
    let index=parseInt(element.attr('data-index'));
    let nextIndex=index+1;
    let nextMinQtyField=$(`input[name="prices[${nextIndex}][qty_min]"]`);
    let nextMaxQtyField=$(`input[name="prices[${nextIndex}][qty_max]"]`);

    if (nextMinQtyField.length>0) {
        let value=parseInt(element.val())+1;
        nextMinQtyField.val(value);
        if (value>nextMaxQtyField.val()) {
            nextMaxQtyField.val(value);
        }
        nextMaxQtyField.attr('min', value);
        nextMaxQtyField.trigger("touchspin.updatesettings", {
            min: value
        });
    }
}