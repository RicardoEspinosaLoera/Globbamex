...
class ProductUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $validations=[
            ...
            'tax' => 'required|min:0',
            'tax_model' => 'required',
            'type_price' => 'required|'.Rule::in(['1', '2']), 
            // 'purchase_price' => 'required|numeric|gt:0',
            'discount' => 'required|gt:-1',
            'shipping_cost' => 'required_if:product_type,==,physical|gt:-1',
            'code' => ['required', 'numeric', 'min:1', 'digits_between:6,20', Rule::unique('products', 'code')->ignore($product->id, 'id')]
        ];

        if ($this->type_price=='2') {
            $validation['prices']='required|array';
            $validation['prices.*']='required|array';
            $validation['prices.*.qty_min']='required|integer|min:1';
            $validation['prices.*.qty_max']='required|integer|min:1';
            $validation['prices.*.price']='required|numeric|regex:/^\d+(\.\d{1,6})?$/|min:0';
        } else {
            $validation['unit_price']='required|numeric|gt:0';
            $validation['minimum_order_qty']='required|numeric|min:1';
        }
