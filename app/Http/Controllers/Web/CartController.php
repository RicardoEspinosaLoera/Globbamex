        ...
        if ($str != null) {
            ...
            for ($i = 0; $i < $count; $i++) {
                if (json_decode($product->variation)[$i]->type == $str) {
                    ...
                    $tax = $product->tax_model=='exclude' ? Helpers::tax_calculation(product: $product, price: $unit_price, tax: $product['tax'], tax_type: $product['tax_type']):0;
                    $update_tax = $tax * $requestQuantity;
                    $discount = Helpers::get_product_discount($product, $unit_price);
                    $price = $unit_price - $discount + $tax;
                    $discountedUnitPrice = $unit_price - $discount;
            ...
            if ($product->type_price=='2') {
                ...
                foreach ($product['prices'] as $key => $value) {
                    ...
                    if ($value->qty_min<=$requestQuantity && $value->qty_max>=$requestQuantity) {
                        $unit_price=$value->price;
                        break;
                    }
                }

                $unit_price=($unit_price<=0 && isset($product['prices']) && isset($product['prices'][0])) ? $product['prices'][0]->price : $unit_price;
                $quantity=($product->current_stock<=$qty_max) ? $qty_max : $product->current_stock;
            } else {
