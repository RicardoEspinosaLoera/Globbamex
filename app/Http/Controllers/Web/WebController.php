...
//
    ...
    public function subscription(Request $request)
    {
        ...
        $subscriptionEmail = SubscriptionEmail::where('email', $request['subscription_email'])->first();

        if(isset($subscriptionEmail)) {
            Toastr::info(translate('You_already_subscribed_this_site'));
        }else{
            $newSubscription = new SubscriptionEmail;
    ...
    public function showblog($route)
    {
        // Buscar el blog que coincida con la ruta
        $blog = Blog::where('route', $route)->first();

        // Verificar si el blog existe
        if (!$blog) {
            return redirect()->back()->with('error', 'No se encontró el blog.');
        }

        // Retornar la vista con el blog encontrado
        return view('admin-views.blogs.showblog', compact('blog'));
    }
   /* public function blog_one()
    {
        $blog = Blog::first();
        return view('admin-views.blogs.blog_one', compact('blog'));
    }
    public function blog_two()
    {
        // Obtener el segundo registro
        $blog = Blog::skip(1)->take(1)->first();
        ...
        return view('admin-views.blogs.blog_two', compact('blog'));
    }
    public function blog_three()
{
    // Obtener el tercer registro
    $blog = Blog::skip(2)->take(1)->first();
    ...
    return view('admin-views.blogs.blog_three', compact('blog'));
}*/
}
