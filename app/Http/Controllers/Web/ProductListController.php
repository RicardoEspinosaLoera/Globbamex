...
class ProductListController extends Controller
{
    ...
    public function default_theme($request) {
        ...
        if ($request->has('name') && !empty($request->input('name'))) {
            $porduct_data->where('name', 'LIKE', '%' . $request->input('name') . '%');
        }
        ...
        //codigo para filtro
        $ignoredWords = ['de', 'con', 'el', 'la', 'los', 'las', 'y', 'a', 'en']; // Aade más palabras si lo deseas
        ...
        if ($request['data_from'] == 'search') {
            ...
            //filtro
            $keywords = array_filter(explode(' ', $request['name']), function ($word) use ($ignoredWords) {
                return !in_array(strtolower($word), $ignoredWords); // Convertir a minúsculas para comparar correctamente
            });

            $product_ids = Product::where(function ($q) use ($keywords) {
