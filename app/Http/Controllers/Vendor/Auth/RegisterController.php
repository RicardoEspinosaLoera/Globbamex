...
class RegisterController extends BaseController
{
    ...
    public function getView():View|RedirectResponse
    {
        ...
        return view(VIEW_FILE_NAMES[Auth::VENDOR_REGISTRATION[VIEW]], compact('plans'));
    }
    public function add(VendorAddRequest $request):JsonResponse
    {
        ...
        $shop = $this->shopRepo->add($this->shopService->getAddShopDataForRegistration(request: $request, vendorId: $vendor['id']));
        $this->vendorWalletRepo->add($this->vendorService->getInitialWalletData(vendorId: $vendor['id']));

        $data = [
            'name' => $request['f_name'],
            'gender' => translate('gender').': '.$vendor->gender,
            'years21' => translate('years21').': '.$vendor->years21,
            'document' => $vendor->document,
            'constancy' => $shop->constancy,
            'status' => 'pending',
        ...
        if ($this->vendorService->isLoginSuccessful($request['email'], $request['password'], NULL)) {
            $plan=$this->planRepo->getFirstWhere([['id', request('plan')]]);
            return response()->json(['redirectRoute' => route('vendor.checkout-subscription', ['plan' => $plan->id])]);
        } else {
            return response()->json(['redirectRoute' => route('vendor.auth.login')]);
        }
    }
}
