...
class ProductService
{
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        if ($request['type_price']=='2') {
            if (isset($request['prices']) && isset($request['prices'][0]) && isset($request['prices'][0]['qty_min'])) {
                $minimum_order_qty=$request['prices'][0]['qty_min'];
            } else {
                $minimum_order_qty=1;
            }
        } else {
        ...
        return [
            ...
            'unit_price' => ($request['type_price']=='1') ? currencyConverter(amount: $request['unit_price']) : 0.00,
            'purchase_price' => 0,
