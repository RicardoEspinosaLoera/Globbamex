...
class ProductRepository implements ProductRepositoryInterface
{
    ...
    public function updatePrices(object $request, object $product): void
    {
        $this->productPrice->where('product_id', $product->id)->delete();

        if ($request['type_price']=='2') {
            foreach ($request['prices'] as $key => $value) {
                $this->productPrice->create(['qty_min' =>  $value['qty_min'], 'qty_max' => $value['qty_max'], 'price' => $value['price'], 'product_id' => $product->id]);
            }
        }
    }
}
