    ...
    */
    ...
    public static function seller_sales_commission($seller_is, $seller_id, $order_total)
    {
        ...
        if ($seller_is == 'seller') {
            ...
            if (isset($seller) && !is_null($seller->subscriptions->first()) && $seller->subscribed($seller->subscriptions->first()->type)) {
                $subscription_api_id = $seller->subscriptions->first()->stripe_price;
                $plan = Plan::where('subscription_api_id', $subscription_api_id)->first();
                if (!is_null($plan)) {
                    $commission = $plan->sales_commission;
                } else {
