        ...
        if ($str != null) {
            ...
            if ($product->type_price=='2') {
                foreach ($product['prices'] as $key => $value) {
                    if ($value->qty_min<=$request['quantity'] && $value->qty_max>=$request['quantity']) {
                        $price=$value->price;
                        break;
                    }
                }

                $price=($price<=0 && isset($product['prices']) && isset($product['prices'][0])) ? $product['prices'][0]->price : $price;
            } else {
